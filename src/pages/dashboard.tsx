...
import { getExcelImports } from 'apiSdk/excel-imports';
import { ExcelImportInterface } from 'interfaces/excel-import';
import { getDeliveryItems } from 'apiSdk/delivery-items';
...
import { ExcelImportListPage as ShowCaseList } from 'pages/excel-imports';

function DashboardPage() {
  const { session } = useSession();

  const excelImportStat = useSWR<PaginatedInterface<ExcelImportInterface>>(
    () => `/excel-imports`,
    () => getExcelImports({ limit: 1, offset: 0 }),
  );

  const deliveryItemStat = useSWR<PaginatedInterface<DeliveryItemInterface>>(
    () => `/delivery-items`,
    () => getDeliveryItems({ limit: 1, offset: 0 }),
  );

  const deliveryNoteStat = useSWR<PaginatedInterface<DeliveryNoteInterface>>(
    () => `/delivery-notes`,
    () => getDeliveryNotes({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Excel Imports',
      result: excelImportStat?.data?.totalCount,
      isLoading: excelImportStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Delivery Items',
      result: deliveryItemStat?.data?.totalCount,
      isLoading: deliveryItemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Delivery Notes',
      result: deliveryNoteStat?.data?.totalCount,
      isLoading: deliveryNoteStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Excel Imports
                </Text>
                <NextLink href={`excel-imports`} passHref legacyBehavior>
                  <Button
